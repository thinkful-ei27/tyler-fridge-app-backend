...
const Item = require('../models/pantryItem');
...
describe('fridge App API - pantry items', function () {
  ...
  describe('GET /api/pantry', function () {
    ...
    it('should return a list of items', function () {
      return Promise.all([
        ...
        chai.request(app).get('/api/pantry')
  ...
  describe('POST /api/pantry', function () {
  ...
  describe('DELETE /api/pantry/:id', function () {
    ...
    it('should delete an existing Item and respond with 204', function () {
      ...
      return Item.findOne({userId: user.id})
        .then(_data => {
          data = _data;
          return chai.request(app).delete(`/api/pantry/${data.id}`)
            .set('Authorization', `Bearer ${token}`);
        })
        .then(function (res) {
    ...
    it('should respond with a 400 for an invalid id', function () {
      return chai.request(app)
        .delete('/api/pantry/NOT-A-VALID-ID')
        .set('Authorization', `Bearer ${token}`)
        .then(res => {
          expect(res).to.have.status(400);
